 ...
 */
class CustomerActLogsController extends AppController
{
    ...
    public function actLogs() {
        ...
        if(!empty($data['customer_wechat'])){
            $where['customer_wechat LIKE'] = '%' . $data['customer_wechat'] . '%';
        }
        ...
        $result= $this->CustomerActLogs->find()
            ->where($where)
            ->order(['id DESC'])
            ->limit($pageSize)
            ->page($page);
