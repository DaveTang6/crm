<template>
	<MainTitle :data-menu="menu" data-active="0" >产品类别</MainTitle>
	<MainBody>
			  <el-tree
...
<script>
	import {getCategories, moveCategory, removeCategory} from '@/api/products'
export default {
	data () {
		return {
			menu: [{
	...
	methods: {
		...
		append(pid) {
			this.$router.push('/product/addCategory/' + pid)
		},
		update(id) {
			this.$router.push('/product/updateCategory/' + id)
		},
		getList () {
			getCategories()
				.then(res => {
