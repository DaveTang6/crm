<template>
	<MainTitle :data-sub-title="alias + '禁用权限'" :data-back="true">管理组</MainTitle>
  <MainRow>
...
export default {
    ...
    handleAppend() {
      ...
      let data = {id: this.$route.params.id, denied: JSON.stringify(a)}
    ...
    handleCheckedAcosChange(pid) {
      ...
      getView({id : this.$route.params.id}).then(data => {
        this.checkedAcos = data.memo.denied
        console.log(data)
        getCategories()
            .then(res => {
