<template>
	<MainTitle data-sub-title="全部" >统计</MainTitle>
...
<script>
import {customerAll, orderAll} from "@/api/statistic"
...
export default {
  ...
  mounted() {
    ...
    getCustomerStatistic () {
      ...
      customerAll({'customerTime': this.customerTime})
          .then(res => {
    ...
    getOrderStatistic () {
      ...
      orderAll({'orderTime': this.orderTime})
          .then(res => {
            this.orderStatistic = res.memo
          })
    },
    //type: y(current year), m(current month), w(current week), 3m(three month), 6m(six month)
    getPeriod(type = 'm'){
