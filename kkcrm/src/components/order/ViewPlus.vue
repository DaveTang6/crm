<template>
  ...
  <MainBody  v-else-if="!loading">
      ...
      >
        ...
        <el-descriptions-item>
          <template #label>
            <div class="cell-item-bold">成本(元)</div>
          </template>
          <span class="cell-item-bold">{{  form.cost}}</span>
        </el-descriptions-item>
        <el-descriptions-item>
          <template #label>
            <div class="cell-item-bold">利润(元)</div>
          </template>
          <span class="cell-item-bold">{{(form.price-form.cost-form.refund).toFixed(2)}}</span>
        </el-descriptions-item>
        <el-descriptions-item>
          <template #label>
...
<script>
	import {  orderViewPlus} from '@/api/orders'
...
export default {
  ...
  mounted() {
    ...
    getView () {
      orderViewPlus({id: this.$route.params.id})
          .then(res => {
              this.loading = false
              this.form = res.memo
