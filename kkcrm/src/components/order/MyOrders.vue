<template>
	<MainTitle :data-menu="menu" data-active="0"  data-sub-title="我的订单">订单管理</MainTitle>
  <OperationContainer
        ...
        >
          <template #default="scope">
            <el-button
                ...
                v-if="scope.row.pay_status==1">查看</el-button>
            <el-dropdown split-button type="primary" size="mini" v-else>
              设置
              <template #dropdown>
                <el-dropdown-menu>
...
<script>
	import {orderMines, orderDelete, orderCheckLogs} from '@/api/orders'
...
export default {
  components:{
    ...
    handleEdit(id) {
      this.$router.push('/order/update/' + id)
    },
		handleDelete(id) {
